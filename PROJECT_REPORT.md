...
- Optional `node-cron` runs every minute, calls internal logic to check due reminders.
  Near-Term Extensions: Streak & window analytics (7/30-day), progress dashboard, multi-channel notifications (Web Push/Email/SMS), user accounts & authentication, rate limiting & request logging.
...
- `ENABLE_LOCAL_CRON=1`: (Optional) Enables local in-process node-cron scheduler.
- `PORT` (optional): Development server port.
- `REQUEST_LOGGING=1`: Enable HTTP request logging via morgan.
- `WEB_PUSH_PUBLIC_KEY`, `WEB_PUSH_PRIVATE_KEY`: VAPID keys for Web Push.
- `SENDGRID_API_KEY`, `SENDGRID_FROM`: SendGrid email sending.
- `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM`: Twilio SMS sending.
- `TEST_MONGO_URI`: Separate MongoDB URI for automated tests.
...
- Input validation prevents malformed data.
  Current: Console logging placeholder when a reminder becomes due; pluggable service stubs in `services/notifications.js`.
  Planned: Web Push (Service Worker + VAPID), Email (SendGrid), SMS (Twilio). Each channel returns success/skip/error objects; future persistence of delivery receipts for analytics.
...
- CORS currently permissive; restrict origins for production.
- Input validation prevents malformed data.
- Rate limiting applied to mutating endpoints; request logging toggle via env.
- Future: Authentication (JWT/session), RBAC for multi-user, advanced monitoring & alerting.
...
- Stats endpoint performs lightweight in-memory streak/window calculations using Set lookups (O(n + d)).
  Next Tests:
...
- Conditional stats test (skips if Mongo unavailable) for streak logic.
  Next Tests:
- Reminder creation & validation failure cases.
...
- **Window Completion**: Percentage of days in last N (7/30) with recorded completion.
...
3. Ethan Brown – _Web Development with Node and Express_, O’Reilly, 2nd Ed.
